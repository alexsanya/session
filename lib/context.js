...
class ContextSession {
  ...
  async initFromExternal() {
    ...
    // ignore cookies but check for key in external source
    // const externalKey = (opts.keySource && ctx[opts.keySource]) ? ctx[opts.keySource] : false; // ctx.cookies.get(opts.key, opts);
    const externalKey = opts.keySource ? opts.keySource(ctx) : false;
    // debug('get external key from cookie %s', externalKey);
    debug('get external key from external source %s', externalKey);
  ...
  initFromCookie() {
    ...
    const cookie = false; // ctx.cookies.get(opts.key, opts);
    if (!cookie) {
      this.create();
      return;
    }
  ...
  create(val, externalKey) {
    ...
    if (this.store) this.externalKey = this.opts.genid(this.ctx);
    this.session = new Session(this.ctx, this.externalKey, val);
  }
  ...
  async commit() {
    ...
    if (!session._requireSave) {
      ...
      if (!prevHash && !Object.keys(json).filter(key => key !== 'sessionId').length) return;
